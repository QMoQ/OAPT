...
class HAB(nn.Module):
    ...
    def forward(self, x, x_size, rpi_sa, attn_mask):
        ...
        # partition windows
        x_windows, h1, w1 = window_partition(shifted_x, self.window_size)  # nw*b, window_size, window_size, c
        ...
        shifted_x = window_reverse(attn_windows, self.window_size, h1, w1)  # b h' w' c

        # reverse cyclic shift
        if self.shift_size > 0:
            attn_x = torch.roll(shifted_x[:,:h,:w,:], shifts=(self.shift_size, self.shift_size), dims=(1, 2))
        else:
            attn_x = shifted_x[:,:h,:w,:]
        attn_x = attn_x.contiguous().view(b, h * w, c)
...
class HAB_offset(nn.Module):
    ...
    def parts_attn(self,x1,rpi, attn_mask):
        if x1 is not None:
            B, H1, W1, C = x1.shape
            # cyclic shift
            if self.shift_size > 0:
            ...
            # self-attention
            x1 = self.attn(x1.contiguous().view(-1, self.window_size * self.window_size, C), rpi, mask=mask1) #
            del mask1
            # merge windows
    ...
    def forward(self, x, x_size, rpi, attn_mask, offset):
        ...
        if offset_h+offset_w > 0:
            ...
            x1 = self.parts_attn(x1, rpi, attn_mask)
            x2 = self.parts_attn(x2, rpi, attn_mask)
            x3 = self.parts_attn(x3, rpi, attn_mask)
            x4 = self.parts_attn(x4, rpi, attn_mask)
            ...
            if self.input_resolution == x_size:
                shifted_x = self.attn(shifted_x.contiguous().view(-1, self.window_size * self.window_size, C),rpi, attn_mask) 
            else:
                if(self.shift_size>0):
                    mask1 = self.calculate_mask([h1,w1]).to(x.device)               
                else:
                    mask1 = None
                shifted_x = self.attn(shifted_x.contiguous().view(-1, self.window_size * self.window_size, C), rpi, mask=mask1)
            # merge windows
...
class OCAB(nn.Module):
    ...
    def forward(self, x, x_size, rpi):        
        ...
        x = self.norm1(x)
        x = x.view(b, H, W, c)
        x = check_image_size_(x, self.window_size)
        b, h, w, c = x.shape
        ...
        # partition windows
        q_windows, _, _= window_partition(q, self.window_size)  # nw*b, window_size, window_size, c
        ...
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1))
        del q,k,kv_windows,q_windows,qkv
        ...
        attn = (attn @ v).transpose(1, 2).reshape(b_, nq, self.dim)

        # merge windows
        # attn_windows = attn_windows.view(-1, self.window_size, self.window_size, self.dim)
        # x = window_reverse(attn_windows, self.window_size, h, w)  # b h w c
        attn = attn.view(-1, self.window_size, self.window_size, self.dim)
        x = window_reverse(attn, self.window_size, h, w)  # b h w c
        x = x[:,:H,:W,:]
        x = x.contiguous().view(b, H * W, self.dim)
...
class OCAB_offset(nn.Module):
    ...
    def forward(self, x, x_size, rpi, offset):
        H, W = x_size
        b, _, c = x.shape
        ...
        if offset_h+offset_w > 0:
            ...
            [x1, x2, x3, x4], h, w = degrade_partition(x, offset, CatMode=True)
            # catmode : window attention
            x1 = self.parts_attn(x1, rpi)
            x2 = self.parts_attn(x2, rpi)
            x3 = self.parts_attn(x3, rpi)
            x4 = self.parts_attn(x4, rpi)
            # merge degraded parts
            attn_x = degrade_reverse([x1,x2,x3,x4],h, w, offset, CatMode=True)
            x = attn_x[:,:H,:W,:]
        else:
            x = self.parts_attn(x, rpi)
...
class AttenBlocks(nn.Module):
    ...
    def __init__(self,
        ...
        self.hab_offset=hab_offset
        self.ocab_offset=ocab_offset
    ...
    def forward(self, x, x_size, params,offset):
        for i,blk in enumerate(self.blocks):
            if not self.hab_offset:
                x = blk(x, x_size, params['rpi_sa'], params['attn_mask'])
            else:
                if(i//2)%2==0:
                    x = blk(x, x_size, params['rpi_sa'], params['attn_mask'])
                else:
                    x = blk(x, x_size, params['rpi_sa'], params['attn_mask'],offset)

        if not self.ocab_offset:
            x = self.overlap_attn(x, x_size, params['rpi_oca'])
        else:
            x = self.overlap_attn(x, x_size, params['rpi_oca'],offset)
...
class RHAG(nn.Module):
    ...
    def __init__(self,
        ...
        self.residual_group = AttenBlocks(
            ...
            hab_offset=hab_offset,
            ocab_offset=ocab_offset)
    ...
    def forward(self, x, x_size, params, offset):
        return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size, params, offset), x_size))) + x
...
class HAT_Offset(nn.Module):
    ...
    def __init__(self,
                ...
                upscale=1, #######
                img_range=1.,
                upsampler='1conv', #####jpeg
                resi_connection='1conv',
                hab_offset=False,
                ocab_offset=False,
                 **kwargs):
        super(HAT_Offset, self).__init__()
    ...
    def calculate_mask(self, x_size):
        ...
        mask_windows, h1, w1 = window_partition(img_mask, self.window_size)  # nw, window_size, window_size, 1
        mask_windows = mask_windows.view(-1, self.window_size * self.window_size)
        attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
        attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))
    ...
    def forward_features(self, x, offset):
        ...
        for layer in self.layers:
            x = layer(x, x_size, params,offset)
