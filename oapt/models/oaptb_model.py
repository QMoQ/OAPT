...
from oapt.models.myb_model import calculate_psnrb
import time
import os

@MODEL_REGISTRY.register()
class OAPT_ModelB(BaseModel): 
    def __init__(self, opt):
        super(OAPT_ModelB, self).__init__(opt) 
        # define network
        self.net_g = build_network(opt['network_g'])
        self.net_g = self.model_to_device(self.net_g)
        self.print_network(self.net_g)
        self.h_time = time.localtime(time.time())
    ...
    def load_networks(self, net, load_path, strict=True, param_key='params'):
        if self.opt['network_g']['type'] == 'OAPT_vary':
            self.load_network(net, load_path[0], strict, param_key, only_restorator=True)
        else:
    ...
    def load_network(self, net, load_path, strict=True, param_key='params',only_restorator=False):
        """Load network.
        Args:
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.opt['network_g']['type'].find('OAPT')>-1:
                        ...
                        with torch.no_grad():
                            if self.opt['network_g']['type'].find('OAPT')>-1:
                                if y==0 and x==0:
                                    ...
                                    # if (int(torch.round(offset_pred[0][0]))==self.offset[0][0] and int(torch.round(offset_pred[0][1]))==self.offset[0][1]): #1.7529, 2.2471
                                    #     self.accuracy += 1
                                    with open(self.txtfile,"a+") as txtfile:
                                        txtfile.write(f"pred-{self.img_name}:{offset_pred[0]}.\n")
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        self.accuracy = 0.
        txtfile_path = os.path.join('./offset_predictor',self.opt['name'])
        if not os.path.exists(txtfile_path):
            os.makedirs(txtfile_path)
        self.txtfile = f"{txtfile_path}/{dataset_name}_pred_{self.h_time.tm_hour}:{self.h_time.tm_min}.txt"
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            with open(self.txtfile,"a+") as tfile:
                tfile.write(f"{img_name}:\t")
            ...
            if 'tile' in self.opt:
                if self.opt['network_g']['type'].find('Offset')>-1 or self.opt['network_g']['type'].find('OAPT')>-1:
                    self.tile_process_8x8()
                else:
                    self.tile_process()
            else:
        ...
        # print("offset pred accuracy: {:.2f}%".format(self.accuracy*100./len(dataloader)))
